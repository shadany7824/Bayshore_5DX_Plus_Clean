...
export async function getOpponentHistory(carId: number)
{
    ...
    let opponentHistory: wmproto.wm5.protobuf.Car[] = [];
    let inserted = 0;
    ...
    if(findChallenger.length > 0)
    {
        for(let i=0; i<findChallenger.length; i++)
        {
            ...
            if(car)
            {
                ...
                opponentHistory.push(wmproto.wm5.protobuf.Car.create({
                    ...car!
                }));
...
export async function getStampTarget(carId: number, arrayCarId: number[])
{
    ...
    let stampTarget: wmproto.wm5.protobuf.StampTargetCar[] = [];
    ...
    if(getStampTargets.length > 0)
    {
        for(let i=0; i<getStampTargets.length; i++)
        {
            ...
            stampTarget.push(
                wmproto.wm5.protobuf.StampTargetCar.create({
                    car: carTarget!,
...
export async function getChallengers(carId: number)
{
    ...
    let challengers: wmproto.wm5.protobuf.ChallengerCar[] = [];
    ...
    if(getChallengers.length > 0)
    {
        for(let i=0; i<getChallengers.length; i++)
        {
            ...
            challengers.push(
                wmproto.wm5.protobuf.ChallengerCar.create({
                    ghostCar: wmproto.wm5.protobuf.GhostCar.create({
                        car: carTarget!,
...
export async function getGhostCar(car: any, area: number, ramp: number, path: number)
{
    ...
    let lists_ghostcar: wmproto.wm5.protobuf.GhostCar[] = [];
    let arr = [];
    let maxNumber = 0;
    ...
    { 
        ...
        if(arr.indexOf(random) === -1)
        {
            ...
            if(!(ghost_trails))
            { 
                lists_ghostcar.push(wmproto.wm5.protobuf.GhostCar.create({
                    car: car[random]
                }));
            }
            ...
            else
            {
                ...
                lists_ghostcar.push(wmproto.wm5.protobuf.GhostCar.create({
                    car: car[random],
                    nonhuman: false,
                    type: wmproto.wm5.protobuf.GhostType.GHOST_NORMAL,
                }));
            }
        }
    }
...
export async function checkGhostSearchByRegion(ghostLevel: number, regionId: number)
{
    ...
    let playedPlace = wmproto.wm5.protobuf.Place.create({ 
    ...
    let car = wmproto.wm5.protobuf.Car.create({ 
    ...
    lists_ghostcar.push(wmproto.wm5.protobuf.GhostCar.create({
        car: car,
        nonhuman: true,
        type: wmproto.wm5.protobuf.GhostType.GHOST_DEFAULT,
    }));
